...
uniform float time;
uniform sampler2DRect coloredNoise;
uniform vec2 coloredNoiseSize;
...
void main() {
    vec2 pos = texCoord;
    // vec2 block = floor(pos / vec2(16));
    // vec2 pos_noise = block / vec2(64);

    // pos_noise += floor(vec2(time) * vec2(1234.0, 3543.0)) / vec2(64);

    // float block_thresh = pow(fract(time * 1236.0453), 2.0) * 0.2;
    // float line_thresh = pow(fract(time * 2236.0453), 3.0) * 0.7;

    // vec2 pos_r = pos, pos_g = pos, pos_b = pos;

    // if (texture2DRect(coloredNoise, pos_noise).r < block_thresh ||
    //     texture2DRect(coloredNoise, vec2(pos_noise.y, 0.0)).g < line_thresh) {
    //     vec2 dist = (fract(pos_noise) - 0.5) * 0.3;
    //     pos_r += dist * 0.1;
    //     pos_g += dist * 0.2;
    //     pos_b += dist * 0.125;
    // }

    // gl_FragColor.r = texture2DRect(diffuseTexture, pos).r;
    // gl_FragColor.g = texture2DRect(diffuseTexture, pos).g;
    // gl_FragColor.b = texture2DRect(diffuseTexture, pos).b;

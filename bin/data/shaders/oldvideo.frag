// ADAPTED FROM https://www.shadertoy.com/view/Xdl3D8
...
//    uv = gl_TexCoord[0].xy / resolution.xy;
...
//		vec3 image = texture2D( iChannel0, vec2(suv.x, suv.y) ).xyz;
		vec3 image = texture2DRect( diffuseTexture, vec2(suv.x, suv.y) * resolution ).xyz;
        ...
        vec3 oldVideoColor = oldImage * vI;
//        gl_FragColor.xyz = oldImage * vI;

		
		// Add some grain (thanks, Jose!)
#ifdef GRAIN
        oldVideoColor *= (1.0+(rand(uv+t*.01)-.2)*.15);
//        gl_FragColor.xyz *= (1.0+(rand(uv+t*.01)-.2)*.15);
#endif
		
        gl_FragColor.xyz = mix( image, oldVideoColor, amount );
        //gl_FragColor = vec4(1.0, 0.0, 0.0, 1.0);
//        gl_FragColor.xyz = mix( texture2DRect( diffuseTexture, gl_TexCoord[0].xy ).xyz, gl_FragColor.xyz, amount );
        
	}
	else
	{
//		gl_FragColor = texture2D( iChannel0, uv );
        gl_FragColor = texture2DRect( diffuseTexture, uv * resolution );
	}
